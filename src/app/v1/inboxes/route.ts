...
// List all inboxes. Optional: limit (default 50, max 100), address filter for exact match
...
export async function POST(req: Request) {
  ...
  if (await getInboxByAddress(address)) return NextResponse.json({ error: "exists" }, { status: 409 });
  try {
    const inbox = await createInbox(address, body.label || null);
